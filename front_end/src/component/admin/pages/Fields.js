...
const Fields = () => {
    ...
    const getStatusBadge = (status) =>
        status === "available" ? (
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-green-500 text-white text-sm">
                <CheckCircle className="h-3 w-3" /> Hoạt động
            </span>
        ) : (
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-red-500 text-white text-sm">
                <XCircle className="h-3 w-3" /> Bảo trì
            </span>
        );

    // Lọc theo courtType.name
    ...
    // Lấy danh sách loại sân để filter
                ...
                {filteredFields.map((field) => (
                    ...
                    >
                        ...
                        <div className="p-6 space-y-2">
                            <h3 className="text-xl font-bold">{field.variantName}</h3>
                            <p className="text-sm text-gray-600">
                                Loại: {field.courtType?.name || "N/A"}
                            </p>
                            <p className="text-sm text-gray-600">
                                Sức chứa: {field.capacity || "N/A"} người
                            </p>
                            <p className="text-sm text-gray-500">{field.description}</p>

                            <button
                                onClick={() => setShowPriceModal(field.id)}
                                className="w-full py-2 bg-indigo-600 text-white rounded-lg font-semibold flex items-center justify-center gap-2 hover:bg-indigo-700 transition"
                            >
                                <DollarSign className="h-4 w-4" /> Xem bảng giá
                            </button>
                        </div>
                    </div>
                ))}
